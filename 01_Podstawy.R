...
?mean # wyświetli pomoc do metody mean(), czyli średniej
??mean
...
install.packages("readr") # instalacja pakietu o nazwie readr

library(readr) # wczytywanie pakietu
library(readrxxx) # sypie błędem, jeżeli pakietu nie ma

require(readr) # też wczytywanie, ale błędem nie sypie
require(readrxxx) # wyświetla jedynie komunikat
(require(readr)) # zwraca TRUE jeżeli pakiet jest
(require(readrxxx)) # zwraca FALSE jeżeli pakietu nie ma
...
# także jest to odpalane automatycznie co jakiś czas
# lub w momencie gdy brakuje pamięci
# wywołanie tej funkcji zwraca statystyki zużycia pamięci
...
?typeof  # zwraca format w jakim dane są trzymane w pamięci
?class   # zwraca w jaki sposób dane powinny być interpretowane
...
x = c(1,2,3) # skalar w R tak na prawdę jest wektorem, ale zawierającym tylko jeden element
x
typeof(x)
class(x)
...
# cięcie na fragmenty
strsplit('a-b-c','-') # rozbija na fragmenty rozdzielone '-'
...
x
typeof(x) # tak na prawdę do dalej jest wektor zmienno przecinkowy...
...
avg_print <- function(data) { # ta funakjca nie zwraca wartości, jedynie ją wypisuje
  ...
  NULL # ten NULL jest celowo, żeby wynik printa nie został zwrócony
}


avg_print(1:5)
res_print = avg_print(1:5)
res_print # jest NULL, więc nic z tym już dalej nie zrobimy :'-(
sqrt(res_print) # w tym przypadku z obliczoną wartością już nic więcej nie zrobimy
